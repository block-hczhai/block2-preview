...
void TestOneSiteAncillaH8STO6G::test_imag_te(
    ...
    mpo = make_shared<SimplifiedMPO<S, FL>>(mpo, make_shared<RuleQC<S, FL>>(), true);
    ...
    vector<FL> te_energies, noises = {0.0};
    ...
    // Ancilla MPS (fitting)
    shared_ptr<MPS<S, FL>> imps = make_shared<MPS<S, FL>>(n_sites, n_sites - 2, 2);
    ...
    shared_ptr<MovingEnvironment<S, FL, FL>> ime =
        make_shared<MovingEnvironment<S, FL, FL>>(impo, imps, mps_thermal, "COMPRESS");
    ...
    shared_ptr<Expect<S, FL, FL>> ex = make_shared<Expect<S, FL, FL>>(me, bond_dim, bond_dim);
...
TEST_F(TestOneSiteAncillaH8STO6G, TestSU2) {
    ...
    shared_ptr<HamiltonianQC<SU2, double>> hamil = make_shared<HamiltonianQC<SU2, double>>(
...
TEST_F(TestOneSiteAncillaH8STO6G, TestSZ) {
    ...
    shared_ptr<HamiltonianQC<SZ, double>> hamil = make_shared<HamiltonianQC<SZ, double>>(
        vacuum, n_physical_sites, orbsym, fcidump);
    hamil->mu = -1.0;
    hamil->fcidump->const_e = 0.0;
