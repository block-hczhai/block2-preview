...
bool TestDMRG<FL>::_mpi = MPITest::okay();
...
TYPED_TEST(TestDMRG, Test) {
    ...
    shared_ptr<ParallelRule<S, FL>> para_rule_pdm =
        make_shared<ParallelRulePDM1QC<S, FL>>(para_comm);
    ...
    // pmpo = make_shared<CondensedMPO<S, FL>>(pmpo, pmpo->basis, true);
    pmpo = make_shared<SimplifiedMPO<S, FL>>(pmpo, make_shared<Rule<S, FL>>(),
                                             true, false);
    cout << "1PDM MPO simplification end .. T = " << t.get_time() << endl;

    cout << "1PDM MPO parallelization start" << endl;
    pmpo = make_shared<ParallelMPO<S, FL>>(pmpo, para_rule_pdm);
    ...
    vector<FP> noises = {1E-4, 1E-4, 1E-4, 1E-4, 1E-4, 1E-5, 1E-5,
                         1E-5, 1E-5, 1E-5, 1E-5, 1E-5, 1E-5, 1E-6, 0};
    vector<FP> davthrs = {1E-5, 1E-5, 1E-5, 1E-5, 1E-5, 5E-7, 5E-7, 5E-7, 5E-7, 5E-7, 1E-10};
    ...
    GMatrix<FL> dm = expect->get_1pdm(20);
