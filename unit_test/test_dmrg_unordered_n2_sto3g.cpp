...
void TestDMRGUnorderedN2STO3G::test_dmrg(const vector<vector<S>> &targets,
    ...
    for (int i = 0; i < (int)targets.size(); i++)
        for (int j = 0; j < (int)targets[i].size(); j++) {
            ...
            shared_ptr<ParallelMPS<S>> mps = make_shared<ParallelMPS<S>>(hamil.n_sites, 0, 2);
            ...
            mps->parallelize(vector<int> {hamil.n_sites / 3, 2 * hamil.n_sites / 3}, me);

            // DMRG
            shared_ptr<DMRG<S>> dmrg = make_shared<ParallelDMRG<S>>(me, bdims, noises);
            dmrg->iprint = 0;
            dmrg->decomp_type = dt;
            dmrg->noise_type = nt;
