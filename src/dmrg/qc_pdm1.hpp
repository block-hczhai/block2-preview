            ...
            if (pm != n_sites - 1) {
                ...
                if (pm == 0) {
                    ...
                    for (uint16_t k = 0; k < m + 1; k++)
                        for (uint16_t j = m + 1; j < n_orbs; j++) {
                            (*pmop)[p] = pdm1_op[k][j];
                            (*pmexpr)[p] = c_op[k] * d_op[j];
                            p++;
                            (*pmop)[p] = pdm1_op[j][k];
                            (*pmexpr)[p] = exf * (d_op[k] * c_op[j]);
                            p++;
                        }
                } else {
                    // 1*1 : mm / cd
                    (*pmop)[p] = pdm1_op[m][m];
                    (*pmexpr)[p] = b_op[m][m] * i_op;
                    p++;
                    // 2*(n-m-1) : mj(-jm) / cd dc (j > m)
                    for (uint16_t j = m + 1; j < n_orbs; j++) {
                        (*pmop)[p] = pdm1_op[m][j];
                        (*pmexpr)[p] = c_op[m] * d_op[j];
                        p++;
                        (*pmop)[p] = pdm1_op[j][m];
                        (*pmexpr)[p] = exf * (d_op[m] * c_op[j]);
                        p++;
                    }
                }
                // 1*1 : jj / cd (j > m) (last site only)
