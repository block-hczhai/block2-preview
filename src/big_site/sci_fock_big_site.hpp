...
inline void setMatrixToZero(block2::CSRSparseMatrix<S, FL> &mat) {
    mat.factor = 0.0;
...
inline void allocateEmptyMatrices(std::vector<EntryTuple> &vec) {
    for (auto &tupl : vec) {
...
struct SCIFockBigSite<S, FL, typename S::is_sz_t> : public BigSite<S, FL> {
     ...
     */
    SCIFockBigSite(int nOrb, int nOrbThis, bool isRight,
                   const std::shared_ptr<block2::FCIDUMP<FL>> &fcidump,
                   const std::vector<uint8_t> &orbsym, int nMaxAlphaEl,
                   int nMaxBetaEl, int nMaxEl, const vector<vector<int>> &poccs,
                   bool verbose = true)
        : BigSite<S, FL>(nOrbThis), nOrbOther{nOrb - nOrbThis},
          nOrbThis{nOrbThis}, nOrb{nOrb}, isRight{isRight},
        ...
        const auto getPgSym = [&orbsym, offset](const SCIFockDeterminant &a) {
            // Could be improved by checking whether its singly or doubly
            // occupied
            ...
            for (auto &o : occA) {
                ...
                symA = symA ^
                       orbsym[offset + o / 2]; //  / 2 as spin orb -> spat orb
            }
            return symA;
        };
        ...
        if (occs.size() == 0 && nMaxEl <= 0)
            occs = ras_space(isRight, nOrbThis, abs(nMaxAlphaEl),
                             abs(nMaxBetaEl), abs(nMaxEl));
        // CI space
        if (occs.size() == 0) {
            // Prelim build up of quantum numbers: Make them sorted (*without*
            // pg symmetry (handled within loop)!)
            ...
            for (auto &pQN : prelimQNs) {
                ...
                SCIFockDeterminant::generateFCIfragmentSpace(nAlpha, nBeta,
                                                             sOrbThis, fSpace);
                if (hasSymmetry) { // sort symmetry numbers and iterate within
                                   // their sectors
                    std::stable_sort(
            ...
            std::vector<size_t> sortIdx;
            {
                ...
                std::iota(sortIdx.begin(), sortIdx.end(),
            ...
            for (size_t ii = 0; ii < occs.size(); ++ii) {
                ...
                if (N != lastN || M != lastM || pg != lastpg) {
                    ...
                    if (hasSymmetry) {
                        ...
                        // "|", sAdded);
                        if (verbose)
                            cout << sci_detail::strPrintf(fStr, lastA, lastB,
                                                          lastN, lastM, lastpg,
                                                          sAdded)
                                 << endl;
                    } else {
                        // say("#", lastA, lastB, "->", lastN, lastM, "|",
                        // sAdded);
                        if (verbose)
                            cout << sci_detail::strPrintf(fStr, lastA, lastB,
                                                          lastN, lastM, sAdded)
            ...
            if (verbose) {
                ...
                cout << "sizes:" << quantumNumbers.size() << " "
                     << offsets.size() << endl;
            }
        }
        {
    ...
    void fillOp_I(BLSparseMatrix &mat) const {
        ...
        if (!excludeQNs) {
            ...
            // but here, we omit some. Allocation of matrices omitted is done
            // with nnz=0 via inBra See comment at
            ...
            for (int ii = 0; ii < sizBra; ++ii) {
                for (int jj = 0; jj < sizKet; ++jj) {
                    ...
                    std::array<int, 3> cre{-1, -1, -1},
                        des{-1, -1, -1}; // third entry may be accessed although
                                         // it is not used
                    int ncre = 0, ndes = 0;
                    {
                        long long u, b, k;
    ...
    void fillOp_Q(const S &deltaQN, std::vector<entryTuple2> &entries) const {
        ...
        for (int itQN = 0; itQN < qnSiz; ++itQN) {
            ...
            const auto o2Bra =
                offsets[braQN].second; // openMP cant handle auto [...] -.-
            const auto sizBra = o2Bra - o1Bra;
            // TODO: How does this perform for small sizes? Some qnblocks have
            // just 1 state (sizBra=sizKet=1)
#ifdef _SCI_USE_OMP_ON
#pragma omp parallel for collapse(2) default(shared) schedule(dynamic)
#endif
            for (int ii = 0; ii < sizBra; ++ii) {
                    ...
                    if (iD == jD) { // Diagonal
                        ...
                        for (int xx = 0; xx < entrySize; ++xx) {
                            ...
                            for (int i = 0; i < closed.size(); i++) {
                                energy += ints2.intsQ(entries[xx].iOrb,
                                                      entries[xx].jOrb,
                                                      closed[i], closed[i]);
                            }
    ...
    void finalize() const {
        ...
        cout << "# Operator  #counts        tot.time/s <time>  <sparsity>   "
                "#dense "
                "     #sparse  #zero mats  overall memory/MB"
             << endl;
        cout << "#  H  "
             << sci_detail::strPrintf(fStr, totCountsH, timeH,
                                      timeH / totCountsH,
                                      summSparsityH / qnCountsH, numDenseH,
                                      numSparseH, numZeroH, usedMemH / 1e6)
             << endl;
        cout << "#  C  "
             << sci_detail::strPrintf(fStr, totCountsC, timeC,
                                      timeC / totCountsC,
        ...
        if (totCountsD > 0)
            cout << "#  D  "
                 ...
                 << endl;
        cout << "#  R  "
             << sci_detail::strPrintf(fStr, totCountsR, timeR,
                                      timeR / totCountsR,
        ...
        if (totCountsRD > 0)
            cout << "#  RD "
                 << sci_detail::strPrintf(
                        fStr, totCountsRD, timeRD, timeRD / totCountsRD,
                        summSparsityRD / qnCountsRD, numDenseRD, numSparseRD,
                        numZeroRD, usedMemRD / 1e6)
                 << endl;
        if (isRight) {
            cout << "#  P  "
            ...
            if (totCountsPD > 0)
                cout << "#  PD "
                     << sci_detail::strPrintf(
                            ...
                            summSparsityPD / qnCountsPD, numDensePD,
                            numSparsePD, numZeroPD, usedMemPD / 1e6)
                     << endl;
            cout << "#  Q  "
            ...
            if (totCountsA > 0)
                cout << "#  A  "
                     << sci_detail::strPrintf(
                            fStr, totCountsA, timeA, timeA / totCountsA,
                            summSparsityA / qnCountsA, numDenseA, numSparseA,
                            numZeroA, usedMemA / 1e6)
                     << endl;
            cout << "#  AD "
                 << sci_detail::strPrintf(
                        fStr, totCountsAD, timeAD, timeAD / totCountsAD,
                        summSparsityAD / qnCountsAD, numDenseAD, numSparseAD,
                        numZeroAD, usedMemAD / 1e6)
                 << endl;
            cout << "#  B  "
        ...
        auto totalTime = timeH + timeC + timeD + timeR + timeRD + timeP +
                         timePD + timeQ + timeA + timeAD + timeB;
...
#ifdef _SCI_USE_OMP_ON
        auto explainIt = []() {
            cerr << " This may happen if the program links against several "
                    "openMP "
                    "libraries. "
                 << endl;
            cerr << " Check it with 'export OMP_DISPLAY_ENV=\"TRUE\"'" << endl;
            cerr << " This also happens if numpy uses a different openMP "
                    "library "
                    "and is imported *before* block. "
                    "Further, set LD_PRELOAD to the libgomp.so you need OR "
                    "(best "
...
#pragma omp critical
            if (omp_get_thread_num() == 0) {
                ...
                if (iThread != nLoop) {
                    cerr << "SCIFockBigSite::checkOMP: OMP has some weird bug! "
                            "i "
                            "loop "
  ...
  protected:
    ...
    size_t fillCoeffs(sci_detail::COOSparseMat<FL> &cooMat,
        ...
        if (!isSparse) {
            ...
            for (int ii = 0; ii < ompThreads; ++ii) {
                for (const auto &tripl : coeffs(ii, iRow)) {
                    ...
                    matRef(tripl.first.first, tripl.first.second) =
                        tripl.second;
                    if /* constexpr */ (Symmetrize) {
                        assert(mat.m == mat.n);
                        if (tripl.first.first != tripl.first.second) {
        ...
        if (isSparse) {
            ++numSparse;
            //                                     vv bit of overestimation for
            //                                     CSR format
            usedMem +=
                nCount *
                (sizeof(FL) + 2 * sizeof(int)); // int in native implementation
        } else {
            ++numDense;
...
#pragma omp parallel for default(shared) schedule(dynamic)
            ...
            for (int jj = 0; jj < sizKet; ++jj) {
                ...
                if /* constexpr */ (Cop) {
                    applResult =
                        SCIFockDeterminant::applyCreator(closedKet, iOrbL);
                } else {
                    applResult =
            ...
            if /* constexpr */ (Cop) {
                nonZeros += fillCoeffs<false>(smat, allOpCoeffs, 0, mat[sym],
                                              summSparsityC, numSparseC,
                                              numDenseC, usedMemC);
            } else {
                nonZeros += fillCoeffs<false>(smat, allOpCoeffs, 0, mat[sym],
                                              summSparsityD, numSparseD,
                                              numDenseD, usedMemD);
            }
        }
        if (nonZeros == 0) {
            ...
            for (int jj = 0; jj < sizKet; ++jj) {
                ...
                if (phase1 != 0) {
                    if /* constexpr */ (Type == 0) { // A: i j
                        applResult2 = SCIFockDeterminant::applyAnnihilator(
                            closedKet2, iOrbL);
                    } else { // A': j' i'; B: i' j
...
#pragma omp parallel for collapse(2) default(shared) schedule(dynamic)
            ...
            for (int ii = 0; ii < sizBra; ++ii) {
                for (int jj = 0; jj < sizKet; ++jj) {
                    ...
                    if /* constexpr */ (Dagger) {
                        closed =
                            ket.getClosed(); // TODO: Change bra/ket loop...
                    }
                    // FILL the matrix elements
                    /*
                     ...
                     *    // TODO similar t H and P creation should be *much*
                     * better
                     *
                     */
                    auto &Hij = Hijs[iThread];
                    memset(Hij.data(), 0, Hij.size() * sizeof(FL));
                    // vv this is the parity coming from applying ll destruction
                    // to ket.
                    //      Since ll is looped from 0 to nOrb, I flip the sign
                    //      for every occupied orbital l
                    FL signL = 1.;
                    ...
                    for (int ll = 0; ll < 2 * nOrbThis; ++ll) {
                        ...
                        if (ket == bra) {
                            ...
                            for (int xx = 0; xx < entrySize; ++xx) {
                                ...
                                FL energy = 0.0;
                                for (int i = 0; i < closed.size(); i++) {
                                    energy += ints2.intsR(iOrb, ll, closed[i],
                                                          closed[i]);
                                }
            ...
            for (int xx = 0; xx < entrySize; ++xx) {
                if /* constexpr */ (Dagger) {
                    nonZeros[xx] += fillCoeffs<false>(
                        smat, allOpCoeffs, xx, entries[xx].mat[sym],
                        summSparsityRD, numSparseRD, numDenseRD, usedMemRD);
                } else {
                    nonZeros[xx] += fillCoeffs<false>(
                        smat, allOpCoeffs, xx, entries[xx].mat[sym],
                        summSparsityR, numSparseR, numDenseR, usedMemR);
                }
            }
        }
        for (int xx = 0; xx < entrySize; ++xx) {
...
#pragma omp parallel for collapse(2) default(shared) schedule(dynamic)
            ...
            for (int ii = 0; ii < sizBra; ++ii) {
                for (int jj = 0; jj < sizKet; ++jj) {
                    ...
                    // ATTENTION: For CISD, this can be simplified. bra = vac
                    // and ket = |kl>
                    //          => occupied orbitals of ket
                    {
                        std::array<int, 3> chg{
                            -1, -1, -1}; // third entry may be accessed
                                         // although it is not used
                        int nchg = 0;
                        {
                            long long u, k;
                            ...
                            for (int i = 0; i < bra.EffDetLen; i++) {
                                ...
                                if (nchg > 2) {
                                    ...
                                    // so this should not happen May happen for
                                    // space larger than CISD: Particle number
                                    // does not help here
                                    goto doneHereP;
                                }
                            }
                        ...
                        for (int xx = 0; xx < entrySize; ++xx) {
                            const auto integral =
                                -ints2.intsP(entries[xx].iOrb, l,
                                             entries[xx].jOrb, k) +
                                ints2.intsP(entries[xx].iOrb, k,
                                            entries[xx].jOrb, l);
                            Hij = sgn * integral;
                            if (std::abs(Hij) > eps) {
                                allOpCoeffs(iThread, xx)
            ...
            if /* constexpr */ (Dagger) {
                for (int xx = 0; xx < entrySize; ++xx) {
                    nonZeros[xx] += fillCoeffs<false>(
                        smat, allOpCoeffs, xx, entries[xx].mat[sym],
                        summSparsityPD, numSparsePD, numDensePD, usedMemPD);
                }
            } else {
                for (int xx = 0; xx < entrySize; ++xx) {
                    nonZeros[xx] += fillCoeffs<false>(
                        smat, allOpCoeffs, xx, entries[xx].mat[sym],
                        summSparsityP, numSparseP, numDenseP, usedMemP);
                }
            }
        }
        for (int xx = 0; xx < entrySize; ++xx) {
