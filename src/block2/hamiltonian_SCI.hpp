...
namespace block2 {
     ...
     *     @attention: For point group symmetry, make sure that MPSInfo::orbsym is empty!
     *
     * */
    template<typename S>
    struct HamiltonianSCI {
        S vacuum;
        // Site basis
        StateInfo<S> *basis; //!< Vector of size n_sites
        ...
        vector<pair<shared_ptr<OpExpr<S>>, shared_ptr<SparseMatrix<S>>>> *site_norm_ops;
        ...
        HamiltonianSCI(S vacuum, int n_sites, const vector<uint8_t> &orb_sym)
            ...
            basis = new StateInfo<S>[n_sites];
            site_op_infos =
                    new vector<pair<S, shared_ptr<SparseMatrixInfo<S>>>>[n_sites];
        ...
        void filter_site_ops(uint16_t m, const vector<shared_ptr<Symbolic<S>>> &mats,
            ...
            // hrl: ops is empty initially. It will be filled here. First by specifying the keys, then by declaring the value
                    ...
                    switch (x->get_type()) {
                        ...
                        case OpTypes::Sum:
                            for (auto &r : dynamic_pointer_cast<OpSum<S>>(x)->strings)
                                ops[abs_value((shared_ptr<OpExpr<S>>) r->get_op())] = nullptr;
                            break;
                        default:
                            assert(false);
                    }
                }
            const shared_ptr<OpElement<S>> i_op = make_shared<OpElement<S>>(OpNames::I, SiteIndex(), vacuum);
            ops[i_op] = nullptr;
            ...
            for (auto pmat : pmats)
                for (auto &x : pmat->data) {
                    ...
                    switch (x->get_type()) {
                        ...
                        case OpTypes::Sum:
                            kk = 0;
                            for (size_t i = 0; i < dynamic_pointer_cast<OpSum<S>>(x)->strings.size(); i++) {
                                // TODO hrl why abs_value? => to remove "phase" for getting key in ops
                                xx = abs_value((shared_ptr<OpExpr<S>>) dynamic_pointer_cast<OpSum<S>>(
                                        x)->strings[i]->get_op());
                                shared_ptr<SparseMatrix<S>> &mat = ops[xx];
                                if (!(mat->factor == 0.0 || mat->info->n == 0)) {
                                    if (i != kk)
                                        dynamic_pointer_cast<OpSum<S>>(x)->strings[kk] = dynamic_pointer_cast<OpSum<S>>(
                                                x)->strings[i];
                                    kk++;
                                }
                            }
                            if (kk == 0)
                                x = zero;
                            else if (kk != dynamic_pointer_cast<OpSum<S>>(x)->strings.size())
            ...
            for (auto pmat : pmats)
                if (pmat->get_type() == SymTypes::Mat) {
                    shared_ptr<SymbolicMatrix<S>> smat = dynamic_pointer_cast<SymbolicMatrix<S>>(pmat);
                    size_t j = 0;
    ...
    protected:
        ...
        shared_ptr<SparseMatrixInfo<S>> find_site_op_info(S q, uint16_t iSite) const {
