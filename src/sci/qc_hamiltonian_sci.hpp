 ...
 */
    ...
    struct HamiltonianQCSCI<S, typename S::is_sz_t> : HamiltonianSCI<S> {
        ...
        map<OpNames, shared_ptr<SparseMatrix<S>>> op_prims_normal[4]; //!< Primitive operators for normal spatial orbitals
        ...
        HamiltonianQCSCI(S vacuum, int nOrbCAS, int nOrbExt,
            ...
            for(int iSite = 0; iSite < n_sites-1; ++ iSite) {
                ...
                bas.n_states[0] = bas.n_states[1] = bas.n_states[2] = bas.n_states[3] = 1;
                bas.sort_states();
            }
            ////////////
            // SCI site
            ...
            for(int i = 0; i < nOrbCas + nOrbExt; ++i){
                if(orb_sym.at(i) != 0){
                    ...
                    throw std::runtime_error("symmetry not yet properly implemented. ");
                }
            }
            const auto iSym = 0;
    ...
    protected:
        void get_site_ops(uint16_t m,
                          map<shared_ptr<OpExpr<S>>, shared_ptr<SparseMatrix<S>>, op_expr_less<S>> &ops) const override {
            if(m == n_sites -1 and true){
                get_site_ops_big_site(ops);
                return;
            }
            uint16_t i, j, k; uint8_t s;
            ...
            for (auto &p : ops) {
                ...
                switch (op.name) {
                    case OpNames::I: // hrl: these are just normal operators so we can apply find_site_norm_op
                    case OpNames::N:
                    case OpNames::NN:
                    case OpNames::C:
                    case OpNames::D:
    ...
    private:
        void init_site_ops() {
            ...
            op_prims_normal[0][OpNames::I]->allocate(find_site_op_info(S(0, 0, 0),0));
            (*op_prims_normal[0][OpNames::I])[S(0, 0, 0)](0, 0) = 1.0; //  |00>
            (*op_prims_normal[0][OpNames::I])[S(1, -1, 0)](0, 0) = 1.0; // |10> (alpha)
            (*op_prims_normal[0][OpNames::I])[S(1, 1, 0)](0, 0) = 1.0; //  |01> (beta)
            (*op_prims_normal[0][OpNames::I])[S(2, 0, 0)](0, 0) = 1.0; //  |11>
            const int sz[2] = {1, -1};
            for (uint8_t s = 0; s < 2; s++) {
                op_prims_normal[s][OpNames::N] = make_shared<SparseMatrix<S>>();
                op_prims_normal[s][OpNames::N]->allocate(find_site_op_info(S(0, 0, 0),0));
                ...
                op_prims_normal[s][OpNames::C]->allocate(find_site_op_info(S(1, sz[s], 0),0));
                (*op_prims_normal[s][OpNames::C])[S(0, 0, 0)](0, 0) = 1.0;
                (*op_prims_normal[s][OpNames::C])[S(1, -sz[s], 0)](0, 0) = s ? -1.0 : 1.0;
                op_prims_normal[s][OpNames::D] = make_shared<SparseMatrix<S>>();
                op_prims_normal[s][OpNames::D]->allocate(find_site_op_info(S(-1, -sz[s], 0),0));
                (*op_prims_normal[s][OpNames::D])[S(1, sz[s], 0)](0, 0) = 1.0;
                (*op_prims_normal[s][OpNames::D])[S(2, 0, 0)](0, 0) = s ? -1.0 : 1.0;
            }
            ...
            for (uint8_t s = 0; s < 4; s++) {
                ...
                op_prims_normal[s][OpNames::NN]->allocate(find_site_op_info(S(0, 0, 0),0));
                ...
                op_prims_normal[s][OpNames::A]->allocate(find_site_op_info(S(2, sz_plus[s], 0),0));
                ...
                op_prims_normal[s][OpNames::AD]->allocate(find_site_op_info(S(-2, -sz_plus[s], 0),0));
                ...
                op_prims_normal[s][OpNames::B]->allocate(find_site_op_info(S(0, sz_minus[s], 0),0));
                opf->product(*op_prims_normal[s & 1][OpNames::C],
                             *op_prims_normal[s >> 1][OpNames::D],
            ...
            for (uint8_t s = 0; s < 4; s++) {
                ...
                op_prims_normal[s][OpNames::R]->allocate(find_site_op_info(S(-1, -sz[s & 1], 0),0));
                ...
                op_prims_normal[s][OpNames::RD]->allocate(find_site_op_info(S(1, sz[s & 1], 0),0));
                opf->product(*op_prims_normal[s & 1][OpNames::C],
            ...
            for (uint16_t m = 0; m < n_sites-1; m++) { // ATTENTION: Not for last site
                for (uint8_t s = 0; s < 2; s++) {
                    ops[orb_sym[m]]
            ...
            for (uint16_t iSite = 0; iSite < n_sites-1; ++iSite) { // ATTENTION: Not for last site
                const auto iSym = orb_sym[iSite];
                site_norm_ops[iSite] = vector<pair<shared_ptr<OpExpr<S>>, shared_ptr<SparseMatrix<S>>>>(
                        ops[iSym].begin(), ops[iSym].end());
                for (auto &p : this->site_norm_ops[iSite]) {
                    OpElement<S> &op = *dynamic_pointer_cast<OpElement<S>>(p.first);
                    p.second = make_shared<SparseMatrix<S>>();
                    p.second->allocate(find_site_op_info(op.q_label, iSite),
        ...
        void init_site_op_infos(){
            // hrl this should be ok for big site as this is the delta quantum number
            ...
            for(int iSite = 0; iSite < n_sites; ++ iSite) {
                ...
                site_op_infos[iSite] =
                        vector<pair<S, shared_ptr<SparseMatrixInfo<S>>>>(info.begin(), info.end());
        ...
        void get_site_ops_big_site(map<shared_ptr<OpExpr<S>>, shared_ptr<SparseMatrix<S>>,op_expr_less<S>> &ops) const {
            ...
            for (auto &p : ops) {
                ...
                switch (op.name) {
                    case OpNames::I: // hrl: these are just normal operators so we can apply find_site_norm_op
                        sciWrapper->fillOp_I(mat); break;
                    case OpNames::N:
                        sciWrapper->fillOp_N(mat); break;
                    case OpNames::NN:
                        sciWrapper->fillOp_NN(mat); break;
                    case OpNames::H:
                        sciWrapper->fillOp_H(mat); break;
                    case OpNames::C:
                        sciWrapper->fillOp_C(dQnPair, mat, ii); break;
                    case OpNames::D:
                        sciWrapper->fillOp_D(dQnPair, mat, ii); break;
                    case OpNames::R:
                        sciWrapper->fillOp_R(dQnPair, mat, ii); break;
                    case OpNames::RD:
                        sciWrapper->fillOp_RD(dQnPair, mat, ii); break;
                    case OpNames::A:
                        sciWrapper->fillOp_A(dQnPair, mat, ii, jj); break;
                    case OpNames::AD:
                        sciWrapper->fillOp_AD(dQnPair, mat, ii, jj); break;
                    case OpNames::B:
                        sciWrapper->fillOp_B(dQnPair, mat, ii, jj); break;
                    case OpNames::P:
                        sciWrapper->fillOp_P(dQnPair, mat, ii, jj); break;
                    case OpNames::PD:
                        sciWrapper->fillOp_PD(dQnPair, mat, ii, jj); break;
                    case OpNames::Q:
                        sciWrapper->fillOp_Q(dQnPair, mat, ii, jj); break;
                    default:
                        assert(false);
                }
            }
